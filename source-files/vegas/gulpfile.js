...
(function () {
    ...
    function tmpCopy() {
        return gulp.src("./dist/**/*").pipe(gulp.dest("./tmp"));
    }
    ...
    function zip() {
        ...
        return gulp.src("./dist/**/*").pipe(plugins.zip(filename)).pipe(gulp.dest("./tmp"));
    }
    ...
    function bump() {
        return gulp
            ...
            .pipe(plugins.bump(/^[a-z]+$/.test(bumpVersion) ? { type: bumpVersion } : { version: bumpVersion }))
            .pipe(gulp.dest("."));
    }
    ...
    function lint() {
        return gulp.src("./src/**.js").pipe(plugins.jshint()).pipe(plugins.jshint.reporter("default"));
    }

    exports.lint = lint;

    function copy() {
        return gulp.src(["./src/**/*", "!./src/sass", "!./src/sass/**"]).pipe(gulp.dest("./dist"));
    }
    ...
    function header() {
        ...
        return gulp.src("./dist/*.js").pipe(plugins.header(settings.banner.content, settings.banner.vars)).pipe(gulp.dest("./dist/"));
    }
    ...
    function changelog(cb) {
        ...
        exec('git log --since="' + lastDate + ' 00:00:00" --oneline --pretty=format:"%s"', function (err, stdout) {
            ...
            const updates = ["### Vegas " + version + " " + date, "", "* " + stdout.replace(/\n/g, "\n* ")].join("\n");
    ...
    const build = gulp.series(lint, clean, copy, sass, header, cssmin, uglify);

    exports.build = build;

    const publish = gulp.series(failIfNotMaster, failIfDirty, tmpCreate, tmpCopy, meta, zip, ghPages, tmpClean);
